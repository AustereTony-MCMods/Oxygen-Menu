...
public class MenuGUIFiller extends BackgroundGUIFiller {
    ...
    public MenuGUIFiller(int xPosition, int yPosition, int width, int height) {             
        super(xPosition, yPosition, width, height, OxygenMenuGUIScreen.MENU_BACKGROUND);
    }
    ...
    public static void drawGradient(int xStart, int yStart, int xEnd, int yEnd, int colorDec1, int colorDec2) {         
        ...
        bufferBuilder.pos((double) xEnd, (double) yEnd, 0.0D).color(red2, green2, blue2, alpha2).endVertex();
        bufferBuilder.pos((double) xEnd, (double) yStart, 0.0D).color(red2, green2, blue2, alpha2).endVertex();
        bufferBuilder.pos((double) xStart, (double) yStart, 0.0D).color(red1, green1, blue1, alpha1).endVertex();
        tessellator.draw();
